...
function App() {
  ...
  const recognize = useCallback(async (sharpenedImage) => {
    ...
    try {
      ...
      switch (outputFormat) {
        ...
        case OutputFormat.DETECTION: {
          boxes = await ocr.getTextBoxes("word");
  ...
  return (
    <div className="app">
      <CameraVideo recognize={recognize} />
      <label for="outputformat-select">Choose output format:</label>
      <select
        name="outputFormat"
      ...
      >
        {Object.keys(OutputFormat).map((k) => (
          <option key={k} value={OutputFormat[k]}>
            {k}
          </option>
        ))}
      </select>
